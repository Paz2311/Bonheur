...
MOT="[Ss][Zz][Cc][Zz][Ęę][Śś][Cc][Ii][Ee]|[Ss][Zz][Cc][Zz][Ęę][Śś][Cc][Ii][Aa]|[Ss][Zz][Cc][Zz][Ęę][Śś][Cc][Ii][Uu]|[Ss][Zz][Cc][Zz][Ęę][Śś][Cc][Ii][Ee][Mm]|[Ss][Zz][Cc][Zz][Ee][Ss][Cc][Ii][Ee]|[Ss][Zz][Cc][Zz][Ee][Ss][Cc][Ii][Aa]|[Ss][Zz][Cc][Zz][Ee][Ss][Cc][Ii][Uu]|[Ss][Zz][Cc][Zz][Ee][Ss][Cc][Ii][Ee][Mm]:?,?"


for file in ../contextes/polonais/pl_*.txt;
do
    lineno=$(echo "$file" | sed -E 's/.*pl_([0-9]+)\.txt/\1/')
    OUTPUT_FILE="../concordances/polonais/pl_${lineno}.html"
    ...
    #grep -o -i -P "(\w+\W+){0,9}${MOT}(\W\w+){0,9}" "$file" | sed -E "s/(.*)(${MOT})(.*)/<tr><td>\1<\/td><td>\2<\/td><td>\3<\/td><\/tr>/" >> "$OUTPUT_FILE"
     #chercher les contextes à l'aide de regex : malheureusement, les deux n'étaient pas performantes, donc on a dû chercher sur internet :
